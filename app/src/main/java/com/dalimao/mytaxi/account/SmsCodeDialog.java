...
public class SmsCodeDialog extends Dialog{
    ...
    static class MyHandler extends Handler{
        ...
        public MyHandler(SmsCodeDialog codeDialog) {
            this.codeDialogRef = new SoftReference<SmsCodeDialog>(codeDialog);
        }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                ...
                case SmsCodeDialog.SMS_SEND_FAIL:
                    ToastUtils.show(dialog.getContext(),dialog.getContext().getString(R.string.sms_send_fail));
                    break;
                case SMS_CHECK_SUC:
                    //验证码校验成功
                ...
                case SMS_SERVER_FAIL:
                    // 服务器错误
                    ToastUtils.show(dialog.getContext(),dialog.getContext().getString(R.string.error_server));
                    break;
            }
        }
    }
                ...
                public void run() {
                    ...
                    if (response.getCode() == BaseBizResponse.STATE_OK){
                        ...
                        if (bizRes.getCode() == BaseBizResponse.STATE_USER_EXIST){
                            mMyHandler.sendEmptyMessage(USER_EXIST);
                        }else if (bizRes.getCode() == BaseBizResponse.STATE_USER_NOT_EXIST){
                            mMyHandler.sendEmptyMessage(USER_NOT_EXIST);
                        }
                    }else{
                        mMyHandler.sendEmptyMessage(SMS_SERVER_FAIL);
                    }
                }
            }.start();
        }
            ...
            public void run() {
                ...
                if (response.getCode() == BaseBizResponse.STATE_OK){
                    ...
                    if (bizRes.getCode() == BaseBizResponse.STATE_OK){
                        mMyHandler.sendEmptyMessage(SMS_SEND_SUC);
                    }else{
                        mMyHandler.sendEmptyMessage(SMS_SEND_FAIL);
                    }
                }else{
                    mMyHandler.sendEmptyMessage(SMS_SEND_FAIL);
                }
        ...
        new Thread(){
            ...
            public void run() {
                ...
                if (response.getCode() == BaseBizResponse.STATE_OK){
                    BaseBizResponse bizRes = new Gson().fromJson(response.getData(),BaseBizResponse.class);
                    if (bizRes.getCode() == BaseBizResponse.STATE_OK){
                        mMyHandler.sendEmptyMessage(SMS_CHECK_SUC);
                    }else{
                        mMyHandler.sendEmptyMessage(SMS_CHECK_FAIL);
                    }
                }else{
                    mMyHandler.sendEmptyMessage(SMS_CHECK_FAIL);
                }
